...
public class CircleSeekbar extends View {
  ...
  private void initPaints() {
    ...
    /**
     * Rounded corner brush at both ends of the selected area
     */
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    if (isCanTouch && (event.getAction() == MotionEvent.ACTION_MOVE || isTouch(x, y))) {
      // Get the cos angle value from the current touch point
      float cos = computeCos(x, y);
      // Obtain an angle value by an inverse trigonometric function
      double angle;
      if (x < getWidth() / 2) { // Sliding over 180 degrees

        angle = Math.PI * RADIAN + Math.acos(cos) * RADIAN;
      } else { // No more than 180 degrees

        angle = Math.PI * RADIAN - Math.acos(cos) * RADIAN;
      }

      //	Log.e(TAG, "onTouchEvent: greater  "+angle );
      ...
      if (mChangListener != null && (event.getAction() & (MotionEvent.ACTION_MOVE | MotionEvent.ACTION_UP)) > 0) {
        ...
        if (previous_process != mCurProcess) {
          mChangListener.onChanged(this, mCurProcess);
        }
        previous_process = mCurProcess;

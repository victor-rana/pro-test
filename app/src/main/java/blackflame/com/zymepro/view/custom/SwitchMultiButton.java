...
public class SwitchMultiButton extends View {
      ...
      for (int i = 0; i < mTabNum; i++) {
        if (x > perWidth * i && x < perWidth * (i + 1)) {
          ...
          if (onSwitchListener != null) {
            onSwitchListener.onSwitch(i, mTabTexts[i],true);
          }
        }
  ...
  public interface OnSwitchListener {
    void onSwitch(int position, String tabText,boolean isUser);
  }
  ...
  public SwitchMultiButton setSelectedTab(int mSelectedTab) {
    ...
    invalidate();
    if (onSwitchListener != null) {
      onSwitchListener.onSwitch(mSelectedTab, mTabTexts[mSelectedTab],false);
    }
    return this;
  }
